...
API.prototype.getClearSignedMessage = function(address, xpriv, message, cb) {
  ...
  var derived = retrieved.derive("m/" + type + "'/" + network + "'/0'" + address.path.substr(1, 4));
  console.log(derived.privateKey.toAddress());
  var result = Utils.signMessage(message, derived.privateKey.toWIF());

  return cb(result);
};

API.prototype.getMessageVerification = function(message, signature, pubKey) {
  var base58 = Bitcore.encoding.Base58Check;
  var pubKeyDecoded = base58.decode(pubKey);

  console.log(pubKeyDecoded); // Buffer
  console.log(pubKey);  // 16nJTndU6zCUhTjXkbkFMSETqW7HizFeQ
  console.log(pubKeyDecoded.toString('hex')); // 000117e52e18ed42189c360ab964beeba899fbecf6ae9f4ae9 ?

  var result = Utils.verifyMessage(message, signature, pubKeyDecoded);
