...
API.prototype.getClearSignedMessage = function(address, xpriv, message, cb) {
  ...
  var signature = Message(message).sign(derived.privateKey);

  return cb(signature);
};

API.prototype.getMessageVerification = function(message, signature, pubKey, cb) {
  var result = Message(message).verify(pubKey, signature);
  return cb(result);
};
/**
